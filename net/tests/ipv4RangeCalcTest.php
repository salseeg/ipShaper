...
class ipv4RangeCalcTest extends \PHPUnit_Framework_TestCase
{
    ...
    function testBasic(){
        $calc = new ipv4RangeCalc('192.168.0.200', 25);

        $netIp = $calc->get_net_ip();
        $this->assertEquals("192.168.0.128", $netIp);
        $this->assertEquals("192.168.0.129", $calc->get_gate_ip());

        $this->assertEquals("255.255.255.128", $calc->get_mask());
        $this->assertEquals("192.168.0.255", $calc->get_broadcast_ip());
        $this->assertEquals("192.168.0.130 - 192.168.0.254", $calc->get_abons_ips_as_range());
